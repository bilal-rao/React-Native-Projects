...
class App extends React.Component {
    ...
    searchByName = (obj) => {
        return ((obj.patientName.search(this.state.search) >= 0) && (obj.date.search(this.state.date) >= 0)) ? true : false
    }
    cancel() {
        this.setState({ date: '' })
    }
                                        ...
                                        {this.props.patient ?
                                            this.props.patient.filter(this.searchByName).map((data, index) => (
                                                <SwipeRow
                                                    ...
                                                    rightOpenValue={-75}
                                                    left={
                                                        <Button success onPress={() => alert('Patient Name : ' + data.patientName + '\n' + 'Patient Disease : ' + data.patientDisease + '\n' + 'Appointment Date : ' + data.date + '\n' + 'Medicine :' + data.medicine + '\n' + 'Dr.Name :' + data.drName)}>
                                                    ...
                                                    right={
                                                        <Button danger onPress={() => this.props.removeDispatch(data._id)}>
